<?php
/**
 * Franziska Veh functions and definitions.
 *
...
if ( ! function_exists( 'franziskaveh_setup' ) ) :
/**
 ...
 */
function franziskaveh_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on Franziska Veh, use a find and replace
	 * to change 'franziskaveh' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'franziskaveh', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'franziskaveh' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'franziskaveh_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif;
add_action( 'after_setup_theme', 'franziskaveh_setup' );
 ...
 */
function franziskaveh_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'franziskaveh_content_width', 640 );
}
add_action( 'after_setup_theme', 'franziskaveh_content_width', 0 );
 ...
 */
function franziskaveh_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'franziskaveh' ),
		'id'            => 'sidebar-1',
		'description'   => esc_html__( 'Add widgets here.', 'franziskaveh' ),
...
add_action( 'widgets_init', 'franziskaveh_widgets_init' );
...
function custom_query_vars( $query ) {
	if ( !is_admin() && $query->is_main_query() ) {
		if ( get_post_type() == 'project' ) {
			$query->set( array ('posts_per_page' => '-1', 'orderby' => 'menu_order' ) );
			// $query->set( 'order' => 'ASC' );
		}
	}
	return $query;
